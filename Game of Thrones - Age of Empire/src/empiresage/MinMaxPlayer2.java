 ...
 * @author Lydia
 */
public class MinMaxPlayer2 extends Player{
    ...
    */
    public MinMaxPlayer2(HashMap state,int player){
        super(state,player);
    }
    /**
        ...
        if (depth == 0 || this.endGame(stateMM)){ 
            ...
            if(player == 2){//max player
                for(Object xo : this.possibleStates(stateMM,2)){
                    int al = this.alphabeta((HashMap) xo, depth-1, alpha, beta, 1);
                ...
                for(Object xo : this.possibleStates(stateMM,1)){
                    int be = this.alphabeta((HashMap) xo, depth-1, alpha, beta, 2);
